...
- YOLOv1의 네트워크 구조에는 총 24개의 convolutional layers(이미지로부터 특징 추출)와, 2개의 fully connected layers(클래스 확률 & bounding box의 좌표 예측)가 .
...
- 위에서 확인하였듯이 YOLOv5는 다양한 종류의 모델이 있다. 소주병 분류 모델은 실시간으로 소주병을 빠르게 탐지해야 한다. 
- 밑의 표는 객체 탐지 모델인 YOLOv5와 EfficientDet의 성능을 비교 분석한 그래프인데, 거의 모든 YOLOv5 모델이 EfficeientDet의 성능과 비슷하거나 압도하는 것을 볼 수 있다.
- 따라서 우리는 적당한 성능에 FPS가 가장 높은 모델인 YOLOv5s를 사용하기로 결정했다.
